...
func TelemetryTrend(points []model.TelemetryPoint) Trend {
	if len(points) < 2 { return Trend{Direction: "flat"} }
	...
	if denominator == 0 { return Trend{Name: ordered[0].Name, Direction: "flat"} }
	slope := (n*sxy - sx*sy) / denominator
	direction := "flat"
	if slope > 0.0001 { direction = "rising" } else if slope < -0.0001 { direction = "falling" }
...
func FrameTrend(frames []model.CalibrationFrame) Trend {
	...
	for _, frame := range frames { points = append(points, model.TelemetryPoint{ID: frame.ID, AntennaID: frame.AntennaID, Name: "signal", Value: frame.Signal, Unit: "relative", CapturedAt: frame.CapturedAt}) }
...
func WindowTrend(points []model.TelemetryPoint, start, end time.Time) Trend {
	...
	for _, point := range points { if (start.IsZero() || !point.CapturedAt.Before(start)) && (end.IsZero() || !point.CapturedAt.After(end)) { filtered = append(filtered, point) } }

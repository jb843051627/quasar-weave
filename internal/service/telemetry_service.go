...
func (l *Lab) SummarizeTelemetry(ctx context.Context, antennaID, name string, start, end time.Time) (TelemetrySummary, error) {
	points, err := l.ListTelemetry(context.Background(), antennaID, name, start, end)
	if err != nil {
		return TelemetrySummary{}, err
	}
	series := model.TelemetrySeries{AntennaID: antennaID, Name: name, Points: append([]model.TelemetryPoint(nil), points...)}
